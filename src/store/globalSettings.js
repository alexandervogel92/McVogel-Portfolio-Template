...
const defaultLightThemeSettings = {
    primary: '#1976D2',
    secondary: '#424242',
    accent: '#82B1FF',
    error: '#FF5252',
...
const defaultDarkThemeSettings = {
    primary: '#2196F3',
    secondary: '#607D8B',
    accent: '#FFC107',
    error: '#F44336',
...
export const useGlobalSettingsStore = defineStore('globalSettings', () => {
    ...
    // --- Reactive State ---
    const currentLanguage = ref(loadFromLocalStorage('portfolioLanguage', 'DE'));
    const themeMode = ref(loadFromLocalStorage('portfolioThemeMode', 'dark'));
    const lightThemeSettings = ref(loadFromLocalStorage('portfolioLightThemeSettings', { ...defaultLightThemeSettings }));
    const darkThemeSettings = ref(loadFromLocalStorage('portfolioDarkThemeSettings', { ...defaultDarkThemeSettings }));
    const themeSettings = computed(() => themeMode.value === 'light' ? lightThemeSettings.value : darkThemeSettings.value);
    // --- Computed ---
   ...
   function toggleTheme() {
    ...
    function loadGoogleFont(fontFamily) {
        ...
        if (isDefaultFont.value || loadedFonts.has(fontFamily))
            return;
        ...
        if (!fontName || fontName.toLowerCase() === 'sans-serif' || fontName.toLowerCase() === 'monospace')
            return;
